...
module.exports = function (app) {
    ...
    plugin.start = function (options) {
        ...
        if (configuration.enableDataPublishing) {
            setupDataPublishing();
        }

        if (configuration.enableEmulator) vesper.start(app, configuration, selfMmsi, selfName, selfCallsign, selfType);
    };
    ...
    plugin.stop = function () {
        ...
        if (configuration.enableEmulator) vesper.stop();
    };
    ...
    plugin.registerWithRouter = (router) => {
        ...
        router.get('/load-configuration', (req, res) => {
            app.debug('load-configuration returning', configuration);
            res.json(configuration);
        });

        // /plugins/${plugin.id}/save-configuration
        router.put('/save-configuration', (req, res) => {
            var newConfiguration = req.body;

            // do some basic validation to ensure we have some real config data before saving it
            if (!newConfiguration
                || !newConfiguration.current
                || !newConfiguration.anchor
                || !newConfiguration.harbor
                || !newConfiguration.coastal
                || !newConfiguration.offshore
            ) {
                app.error('ERROR - not saving invalid new configuration', newConfiguration);
                res.status(500).end();
                return;
            }
            app.debug('save-configuration saving', newConfiguration);
            app.savePluginOptions(newConfiguration, () => { app.debug('configuration saved') });
            res.json();
            // update the config that the live executing plugin is using
            configuration = newConfiguration;
        });

    };

    function setupDataPublishing() {
