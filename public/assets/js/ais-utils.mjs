...
function updateDerivedData(targets, selfTarget, collisionProfiles, TARGET_MAX_AGE) {
    ...
    targets.forEach((target, mmsi) => {
        if (mmsi != selfTarget.mmsi) {
            updateSingleTargetDerivedData(target, selfTarget, collisionProfiles, TARGET_MAX_AGE);
        }
    });
}
...
function updateSingleTargetDerivedData(target, selfTarget, collisionProfiles, TARGET_MAX_AGE) {
    ...
    target.x = target.longitude * 111120 * Math.cos(toRadians(selfTarget.latitude));
...
function calculateRangeAndBearing(selfTarget, target) {
    ...
    target.range = Math.round(getDistanceFromLatLonInMeters(selfTarget.latitude, selfTarget.longitude, target.latitude, target.longitude));
    target.bearing = Math.round(getRhumbLineBearing(selfTarget.latitude, selfTarget.longitude, target.latitude, target.longitude));
